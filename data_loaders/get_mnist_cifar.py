...
def get_data(problem, shards, rank, data_augmentation_level, n_batch_train, n_batch_test, n_batch_init, resolution):
    if problem == 'mnist':
        ...
        import pickle
        with open('data_loaders/galaxy.pkl', 'rb') as f:
            x_train, x_val, x_test = pickle.load(f)

        x_test = np.concatenate([x_val, x_test], axis=0)

        y_train = np.random.random(size=len(x_train)).astype('int32')
        y_test = np.random.random(size=len(x_test)).astype('int32')
    ...
    if data_augmentation_level == 0:
        ...
        if problem == 'space':
            ...
            datagen_train = ImageDataGenerator(
                ...
                vertical_flip=True,
                preprocessing_function=random_transpose
                )
        elif problem == 'mnist':
            datagen_train = ImageDataGenerator(
                width_shift_range=0.1,
            ...
            if data_augmentation_level == 1:
                datagen_train = ImageDataGenerator(
                    width_shift_range=0.1,
                    height_shift_range=0.1,
                    horizontal_flip=True  # Found in old glow code
                )
