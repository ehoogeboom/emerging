...
from conv2d.conv2d import invertible_conv2D_emerging, invertible_ar_conv2D
from conv2d.fourier import fourier_conv
from conv2d.conv1x1 import invertible_1x1_conv
from maf.three_layer import maf_three
'''
...
def abstract_model_xy(sess, hps, feeds, train_iterator, test_iterator, data_init, lr, f_loss):
    ...
    check_op = tf.no_op()  # tf.add_check_numerics_ops()

    train_op, polyak_swap_op, ema = optimizer(
        all_params, gs, alpha=lr, hps=hps)
    if hps.direct_iterator:
        m.train = lambda _lr: sess.run([check_op, train_op, stats_train], {lr: _lr})[2]
    else:
        def _train(_lr):
            _x, _y = train_iterator()
            return sess.run([check_op, train_op, stats_train],
                            {feeds['x']: _x, feeds['y']: _y, lr: _lr})[2]
        m.train = _train
    ...
    m.restore = _restore  # lambda path: saver.restore(sess, path)
...
def codec(hps):
    ...
    def encoder(z, objective):
        ...
        for i in range(hps.n_levels):
            ...
            if i < hps.n_levels-1:
                z, objective, _eps = split2d("pool"+str(i), z, hps, objective=objective)
                eps.append(_eps)
    ...
    def decoder(z, eps=[None]*hps.n_levels, eps_std=None):
        for i in reversed(range(hps.n_levels)):
            if i < hps.n_levels-1:
                z = split2d_reverse("pool"+str(i), z, hps, eps=eps[i], eps_std=eps_std)
...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    with tf.name_scope('input'):
        X = tf.placeholder(
            tf.uint8, [None, hps.image_size, hps.image_size, hps.channels], name='image')
    ...
    with tf.variable_scope(name):
        ...
        if not reverse:
            ...
            if hps.flow_permutation == 0:
                ...
                z, logdet = invertible_1x1_conv("invconv", z, logdet, decomposition=hps.decomposition)
            elif hps.flow_permutation == 3:
                z, logdet = invertible_1x1_conv("invconv", z, logdet, decomposition=hps.decomposition)
                z, logdet = invertible_conv2D_emerging("emerging", z, logdet, checkpoint_fn=checkpoint)
            elif hps.flow_permutation == 4:
                z, logdet = fourier_conv(
                    'fourier', z, logdet)
            elif hps.flow_permutation == 5:
                z, logdet = invertible_1x1_conv("invconv", z, logdet, decomposition=hps.decomposition)
                z, logdet = maf_three('maf1', z, logdet, depth=96, is_upper=False)
                z, logdet = maf_three('maf2', z, logdet, depth=96, is_upper=True)
            else:
                raise Exception()
            ...
            if hps.flow_permutation == 0:
                ...
                z, logdet = invertible_1x1_conv(
                    "invconv", z, logdet, reverse=True, decomposition=hps.decomposition)
            elif hps.flow_permutation == 3:
                z, logdet = invertible_conv2D_emerging(
                    "emerging", z, logdet, reverse=True)
                z, logdet = invertible_1x1_conv(
                    "invconv", z, logdet, reverse=True, decomposition=hps.decomposition)
            elif hps.flow_permutation == 4:
                z, logdet = fourier_conv(
                    'fourier', z, logdet, reverse=True)
            elif hps.flow_permutation == 5:
                z, logdet = maf_three(
                    'maf2', z, logdet, depth=96, is_upper=True, reverse=True)
                z, logdet = maf_three(
                    'maf1', z, logdet, depth=96, is_upper=False, reverse=True)
                z, logdet = invertible_1x1_conv(
...
@add_arg_scope
def split2d(name, z, hps, objective=0.):
    with tf.variable_scope(name):
        n_z = Z.int_shape(z)[3]
        z1 = z[:, :, :, :n_z // 2]
        z2 = z[:, :, :, n_z // 2:]
